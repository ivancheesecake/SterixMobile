...
public class AreaEditFindingActivity extends AppCompatActivity {

    Bitmap imageBitmap;
    Bundle extras;
    String service_order_location, service_order_id,finding,proposedAction,actionsTaken,personInCharge,riskAssessment, status, finding_id,position,photosForDeletionStr,photosFindingsUpload,photosActionTakenUpload;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_area_edit_finding);
...
//            DO STUFF HERE
    ...
    public File createImageFile(String tag) throws IOException {
        ...
        String imageFileName = "JPEG_" + timeStamp + "_"+m.getService_order_id()+"_";
    ...
    public void saveFinding(View v){
        ...
        AreaViewFindingActivity.findings.get(Integer.parseInt(position)).setFinding(findings);
        AreaViewFindingActivity.findings.get(Integer.parseInt(position)).setProposedAction(proposedAction);
        AreaViewFindingActivity.findings.get(Integer.parseInt(position)).setActionTaken(actionsTaken);
        AreaViewFindingActivity.findings.get(Integer.parseInt(position)).setTimestamp(timeStamp);
        AreaViewFindingActivity.findings.get(Integer.parseInt(position)).setRiskAssessment(riskAssessment);
        AreaViewFindingActivity.findings.get(Integer.parseInt(position)).setPersonInCharge(personInCharge);
        AreaViewFindingActivity.findings.get(Integer.parseInt(position)).setStatus(status);

        AreaViewFindingActivity.mAdapter.notifyDataSetChanged();
        ...
        if(isOnline()){
            ...
            String url ="http://"+ip+"/SterixBackend/updateFinding.php";
